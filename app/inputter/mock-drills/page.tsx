...
interface MockDrill {
  ...
  participants: number; drillType: string | null; result: string | null; notes: string | null
  branch: { id: string; name: string }; createdBy: { id: string; name: string }
}

export default function InputterMockDrillsPage() {
  const { user, loading: authLoading } = useAuth()
  const [items, setItems] = useState<MockDrill[]>([])
  ...
  async function fetchData() {
    const res = await fetch("/api/data/mock-drills")
    const data = await res.json()
    setItems(data.mockDrills || []); setLoading(false)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/data/mock-drills", {
        ...
        body: JSON.stringify({ title, description, date, participants, drillType, result, notes }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error); setSubmitting(false); return }
      setSuccess("Mock drill created successfully")
      setTitle(""); setDescription(""); setDate(new Date().toISOString().split("T")[0]); setParticipants(0); setDrillType(""); setResult(""); setNotes("")
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <section className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-sky-950/30 backdrop-blur">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-semibold tracking-tight text-white">Mock Drills</h1>
                <p className="mt-2 text-sm text-slate-300">Record mock drill exercises for {user.branch?.name}</p>
              </div>
              <button onClick={() => setShowForm(!showForm)} className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 px-4 py-3 font-medium text-cyan-100 transition hover:bg-cyan-400/20">
                {showForm ? "Cancel" : "+ New Mock Drill"}
              </button>
            </div>
          </section>
          ...
          {showForm && (
            <section className="rounded-[28px] border border-cyan-400/20 bg-white/5 p-6 backdrop-blur">
              <h2 className="text-lg font-semibold text-white mb-4">New Mock Drill</h2>
              <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-slate-300 mb-1">Title *</label>
                  <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full rounded-xl border border-white/10 bg-slate-950/50 px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:border-cyan-400/50 focus:outline-none" placeholder="Mock drill title" />
                </div>
                <div className="md:col-span-2">
                ...
                <div>
                  ...
                  <select value={drillType} onChange={(e) => setDrillType(e.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-950/50 px-4 py-2.5 text-white text-sm focus:border-cyan-400/50 focus:outline-none">
          ...
          <section className="rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur">
            <h2 className="text-lg font-semibold text-white mb-4">Mock Drills ({items.length})</h2>
            {items.length === 0 ? <div className="text-center py-12"><p className="text-slate-400">No mock drills yet.</p></div> : (
                ...
                {items.map((d) => (
                  <div key={d.id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                    <div className="flex items-center gap-2">
                      ...
                      {d.result && <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase ${resultColors[d.result] || ""}`}>{d.result}</span>}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-500">
                      ...
                      {d.drillType && <span>🚨 {d.drillType}</span>}
                    </div>
                  </div>
                ))}
              </div>
