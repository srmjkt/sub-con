...
export default function InputterIncidentDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/inputter/incidents" className="text-cyan-400 hover:underline">Back to incidents</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      ...
      <main className="ml-64 p-8">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* Back button */}
          <Link href="/inputter/incidents" className="inline-flex items-center gap-2 text-sm text-cyan-400 hover:text-cyan-300 transition">
            ← Back to incidents
          </Link>
          ...
          <section className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-sky-950/30 backdrop-blur">
            <div className="flex items-start justify-between gap-4">
              ...
              <Link
                href={`/inputter/incidents?edit=${incident.id}`}
                className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 px-4 py-2 text-sm font-medium text-cyan-100 transition hover:bg-cyan-400/20"
              >
                Edit This Report
              </Link>
            </div>
